...
class Proxy(Credential):
    ...
    def __init__( self, args ):
        ...
        self.myproxyValidity = args.get(
            "myproxyValidity", "168:00"
        )  # lenght of the myproxy
        self.myproxyMinTime = args.get(
            "myproxyMinTime", 4
        )  # threshold used in checkProxy
        self.myproxyAccount = args.get(
            "myproxyAccount", ""
        )  # to be used when computing myproxy account (-l option)
        self.rfcCompliant = args.get(
            "rfcCompliant", True
        )  # to be used when computing myproxy account (-l option)
